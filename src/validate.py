...
# python src/validate.py --dataset 'FashionIQ' --combining-function 'combiner' --combiner-path models/fiq_comb_RN50x4_fullft.pt --projection-dim 2560 --hidden-dim 5120 --clip-model-name RN50x4 --clip-model-path models/fiq_clip_RN50x4_fullft.pt --target-ratio 1.25 --transform targetpad
...
def compute_fiq_val_metrics(relative_val_dataset: FashionIQDataset, clip_model: CLIP, index_features: torch.tensor,
    ...
    predicted_features, target_names, reference_names_list = generate_fiq_val_predictions(clip_model, relative_val_dataset,
                                                                    combining_function, index_names, index_features)
    # predicted_features (#dress_type, 640) or (2038, 640)
    assert len(reference_names_list) == len(target_names), "ref and target names not equal"
    print(f"Compute FashionIQ {relative_val_dataset.dress_types} validation metrics")  # shirt

    # Normalize the index features
    index_features = F.normalize(index_features, dim=-1).float()  # #samples, 640

    # Compute the distances and sort the results
    distances = 1 - predicted_features @ index_features.T  # (2038, 6346)
    sorted_indices = torch.argsort(distances, dim=-1).cpu()  # (2038, 6346)
    sorted_index_names = np.array(index_names)[sorted_indices]  # (2038, 6346)

    # Compute the ground-truth labels wrt the predictions
    labels = torch.tensor(
        sorted_index_names == np.repeat(np.array(target_names), len(index_names)).reshape(len(target_names), -1))  # create true false mask (2038, 6346)
    assert torch.equal(torch.sum(labels, dim=-1).int(), torch.ones(len(target_names)).int())

    # save results
    labels_df = pd.DataFrame(labels.numpy())
    preds_df = pd.DataFrame(sorted_index_names)

    labels_df['ref_image'] = reference_names_list
    preds_df['ref_image'] = reference_names_list


    labels_df.to_csv(f'results/{relative_val_dataset.dress_types[0]}_labels.csv', index=False)
    preds_df.to_csv(f'results/{relative_val_dataset.dress_types[0]}_preds.csv', index=False)
...
def generate_fiq_val_predictions(clip_model: CLIP, relative_val_dataset: FashionIQDataset,
    ...
    name_to_feat = dict(zip(index_names, index_features))  # {indexname: tensor(640,)}
    ...
    return predicted_features, target_names, reference_names_list
...
def fashioniq_val_retrieval(dress_type: str, combining_function: callable, clip_model: CLIP, preprocess: callable):
    ...
    index_features, index_names = extract_index_features(classic_val_dataset, clip_model)  # index features: (#samples, 640), index_names: list of #samples ['B00kENMD8', ...]
...
def main():
    ...
    if args.combining_function.lower() == 'sum':
        ...
        combiner = Combiner(feature_dim, args.projection_dim, args.hidden_dim).to(device, non_blocking=True)  # 240 2560 5120
